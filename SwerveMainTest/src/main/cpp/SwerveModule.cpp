...
SwerveModule::SwerveModule(const int driveMotorChannel,const int turningMotorChannel): m_driveMotor{driveMotorChannel, brushless}, m_turningMotor{turningMotorChannel, brushless} 
{
  ...
  // to be continuous.
  m_turnPIDController.SetOutputRange(-3.14159, 3.14159);
}
...
void SwerveModule::SetDesiredState(const frc::SwerveModuleState& state) {
  ...
  //const auto driveOutput = m_driveMotor.
  ...
  m_turnPIDController.SetReference(units::radian_t(m_turningEncoder.GetPosition()), rev::ControlType::kPosition);
}